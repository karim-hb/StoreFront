...
const Header = () => {
  ...
  return (
    <header className="flex justify-between items-center flex-wrap p-3 header-shadow sticky px-5 bg-white top-0 z-40">
      <div className="flex items-center">
        ...
        <span className="material-icons-outlined text-lg text-gray-400 mr-12 ml-3">
          location_on
        </span>
        <span className="text-xs text-gray-400">ادرسی خود را انتخاب کنید</span>
        <span className="material-icons-outlined text-[#FF00A6] mr-5 text-lg">
          keyboard_arrow_down
        </span>
      </div>
      <div className="flex items-center rounded-lg background bg-gray-200 p-3 max-w-lg w-full">
        <span className="material-icons-outlined w-[5%] text-xl text-gray-400 text-right">
          search
        </span>
        <input
          placeholder="جستجو در رستوران"
          className="border-0 outline-none w-[95%] bg-gray-200 text-black "
        />
      </div>
      <div className="flex items-center ml-10">
        {userData.user?.isAuthenticated ? (
          <>
            {" "}
            <span className="material-icons-outlined ml-10 text-xl">
              person_outline
            </span>
            <span className="material-icons-outlined  text-xl">
              description
            </span>
            <span className="text-sm mr-3">سفارش ها</span>
          </>
        ) : (
          <button
            onClick={() => setOpenLoginModel(true)}
            className="mainShadow text-sm  bg-[#ff00a6] text-white w-[110px]   py-2 px-4 rounded-3xl"
          >
            ورود
          </button>
        )}
      ...
      >
        <Box sx={style}>
          <Login closeLogin={() => setOpenLoginModel(false)} />
        </Box>
      </Modal>
    </header>
  );
